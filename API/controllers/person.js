...
const createPerson = async (req, res) => {
  ...
  try {
    ...
    if (status == "OKP OKM") {
      existValidation = (await pool.query('SELECT person_id FROM Person WHERE name=$1', [name])).rows;
      if (existValidation.length != 0) {
        res.status(400).json({
          message: 'La persona ya existe en la base de datos',
        });
      } else {
      ...
      res.status(406).json({
        message: 'No se han encontrado los datos de los padres en la base de datos',
      });
    }
  } catch {
